...
- **GeometryEngine +3 ops:** `labelPoint(polygon)` (точка для лейбла внутри полигона), `normalizeCentralMeridian(geometry)` (разворот через антимеридиан), `reshape(geometry, reshaper)` (обвод polyline/polygon линией). **Гоча:** Swift `reshape<Multipart>` — generic по КОНКРЕТНОМУ типу → ветки `as? Polyline`/`as? Polygon` (не `any Multipart`); Kotlin — по базе `Multipart`. `createPointAlong` выкинут (нет в Swift). **Верификация:** TS/Android/iOS ✅.
- **DEFER (GeometryEngine остаток):** `densifyGeodetic`/`nearestCoordinateGeodetic` (нужны unit+curve — addable), `extend`/`tryAutoComplete`/`tryIntersections`/`moveGeodetic`/`sectorGeodesic`/`ellipseGeodesic` (доп.параметры/гео), `createWithZ/M`-билдеры. `DistanceCompositeSceneSymbol` (composite-ranges), `ModelSceneSymbol` (3D-ассет).
