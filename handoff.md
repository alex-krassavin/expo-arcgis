...
## Batch 3 (готово ✅ — TS·Android·iOS зелёные) — contingent values + network server-trust handler

**Исходный Batch 3 (attribute rules / portal content / PKI-IWA) НЕ сложился** — глубокий сёрф 300.0.0: **PKI/IWA** — нет client-cert/`CertificateCredential` класса (network `Disposition` берёт только Token/OAuth/IAP `ArcGISCredential`); **portal content** в runtime во многом **read-only** (`PortalUser.content` — чтение; create/update item'ов нет; `portal.findItems` уже есть). Заменили на 2 реальные SDK-фичи (выбор юзера):

1. **contingent values** — `FeatureLayerRef.contingentValues(attributes, fieldName)`: `ArcGISFeatureTable.contingentValuesDefinition` (Loadable, load) → `makeFeature(attributes:)` → `contingentValues(with:forFieldNamed:) → ContingentValuesResult{contingentValuesByFieldGroup}` → флатим `ContingentCodedValue.codedValue{name,code}`. Каст table→ArcGISFeatureTable (гард как в applyEdits). Для форм редактирования.
2. **server-trust handler** — `setAllowUntrustedHosts(allow)`: ставит `authenticationManager.networkAuthenticationChallengeHandler`, который на `.serverTrust` возвращает trust (`.continueWithCredential(.serverTrust)`), иначе `.continueWithoutCredential`. Замена нереализуемому PKI/IWA (on-prem self-signed). **Security-sensitive — dev/on-prem only** (JSDoc).

Верификация: TS tsc + Android `:expo-arcgis:compileDebugKotlin` + iOS `xcodebuild ExpoArcgis`. DEFER: PKI client-cert + IWA (нет credential класса в runtime); portal item write (read-only); contingent values typed/range constraints (пока coded-values).

**Результат:** обе фичи компилируются TS·Kotlin·Swift. Коммиты: `feb31f6` contingent · `ef129e7` server-trust · `0bae3f8` parity-fix.

**🔴 Восстановление после прерывания сессии:** оба агента были **убиты session-interrupt'ом ДО коммита** (работа осталась незакоммиченной в воркт-ри). Contingent-агент вдобавок **протёк** своими TS-файлами в main (копировал для верификации) → откатил `git checkout HEAD -- types.ts ExpoArcgisModule.ts`, потом закоммитил полную работу из воркт-ри и cherry-pick'нул. Урок: убитый агент = проверять `git -C <worktree> status` (незакоммиченное) + не доверять, что main чист.

**Гочи:**
- **Kotlin `NetworkAuthenticationChallengeResponse` НЕ имеет `ContinueWithoutCredential`** — варианты: `Cancel` / `ContinueAndFail` / `ContinueAndFailWithError` / `ContinueWithCredential`. Swift `.continueWithoutCredential` ≈ Kotlin **`ContinueAndFail`** (агент поставил `.Cancel` → отменял бы basic/ntlm/clientcert; поправил на `.ContinueAndFail` для паритета).
- Contingent-агент добавил **бонусом** `getContingentValues(objectId, fieldName)` (для уже существующей фичи по objectId) рядом с запрошенным `contingentValues(attributes, fieldName)`.
- `makeFeature(attributes:) as! ArcGISFeature` (ArcGISFeatureTable возвращает ArcGISFeature); сериализуются только `ContingentCodedValue` (range/null/any опускаются).
- Новый `NetworkTrustHandler.swift` → iOS требует `pod install`.

**Сэмплы — оба DEFER:** contingent values нужен нишевый сервис с contingent-определениями; server-trust — не-визуальный security-тоггл (нет смысла в галерее). Фичи в `FeatureLayerHandle`/auth-типах + JSDoc (server-trust помечен dev/on-prem only).

## 0.3.1 — отложенные сэмплы добиты + РАНТАЙМ-проверены на устройствах (iOS-сим + Android-эмулятор)

**model-scene** (`ModelSceneSymbol`, 3D glTF) — ✅ **iOS + Android**.
- 🔴 **БАГ найден+пофикшен (Android):** Kotlin `ModelSceneSymbol("file://…")` **молча** не грузит локальный файл (logcat: glb скачан 200, модель не рендерится, без ошибки). iOS `URL(string:)` схему ест, Android — нет. Фикс: `Uri.parse(rawUri).path` (`file://`→путь) в `GraphicsOverlayRef.kt`, iOS не тронут. Коммит `d603ac2`.
- ModelSceneSymbol грузит **только локальный файл** (remote http SDK НЕ качает — подтверждено пустым логом). Сэмпл качает Khronos Duck.glb рантаймом (`expo-file-system/legacy`) → `file://` → символ. Камера: `orbitGeoElement` (z/surfacePlacement-агностично; фикс-камера промахивалась по z).

**contingent-values** (`FeatureLayer.contingentValues`) — ✅ **iOS + Android**.
- Ассет: Esri **bird-nests** гео-БД (AGOL item `e12b54ea799f4606a2712157cf9f6e41`; `/data` = ZIP 28KB → `.geodatabase` 364KB). Бандл через **expo-asset** (`metro assetExts += 'geodatabase'`); ассет в `example/assets` И `test/src/assets` → `require('../../assets/…')` verbatim в обоих. `openGeodatabase(path)` хочет **сырой путь** → strip `file://`.
- Схема `BirdNests`: `Status`(OCCUPIED/UNOCCUPIED) → `Protection`(ENDANGERED/NOT_ENDANGERED/NA) → `BufferSize`. Демо: `contingentValues({Status:'OCCUPIED'},'Protection')` → валидные коды (статус-текст верифицирован).

**KML-tour** (`playTour/pauseTour/resetTour`) — ✅ **iOS**, 🔴 **Android = ограничение SDK** (задокументировано, не пофикшено).
- iOS: авто-play (`getNodes()`→settle→`playTour`) + кнопки. KMZ с `gx:Tour` пишется строкой в локальный файл рантаймом.
- **Android не двигает камеру даже по кнопке.** Корень: либа на Android — **view-based `SceneView`**, у него (и у `GeoView`) **нет** tour-членов; `KmlTourController` отдаёт лишь `currentPosition`/`status` (камеры/geoView нет → вручную гнать нельзя). Авто-драйв туров есть **только в Compose `SceneView`**; на iOS SwiftUI-`SceneView` подхватывает сам. Фикс = `ExpoArcgisSceneView.kt` view-based → Compose (крупно, **DEFER**). Помечено iOS-only в `KmlLayerHandle` JSDoc + сэмпле. Коммит `93e871a`.

**Харнесс (computer-use юзер отключил):** temp `<Redirect>` в `index.tsx` на сэмпл (no-tap) + `simctl`/`adb screencap`.
- 🔴 **`timeout` НЕ установлен в шелле** (macOS без coreutils) — `timeout adb …` падали «command not found» → пустой вывод → ложное «эмулятор не виден»/«screencap timeout». Чистый `adb` работает.
- adb видит свежий эмулятор только после `adb kill-server && start-server` **ПОСЛЕ** boot_completed (adbd внутри поднят); рестарт adb раньше не помогает. Android-апп → нужен `adb reverse tcp:8081 tcp:8081` до Metro.
