...
**Остаётся по 3D-списку:** Display a web scene (`portalItem` на `<Scene>`), Change camera controller (рантайм-камера на `<SceneView>`, как `viewpoint` у MapView), Scene layer (`ArcGISSceneLayer`/`BuildingSceneLayer`), light & shadows, image overlay, mobile scene package, AR tabletop, local scene.
