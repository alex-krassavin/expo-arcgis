...
- **DEFER (нужен особый setup):** `BingMapsLayer` (Swift `BingKey`-wrapper + Bing Maps API ретайрится), `EncLayer` (ENC exchange set/cell), GeoPackage-слой (async-load → swap), `CustomWebTiledLayer`/`ImageTiledLayer`/`ServiceImageTiledLayer`/`CustomTiledLayer` (tile-info/abstract), `MobileBasemapLayer`/`Unknown`/`Unsupported` (internal). Камеры: orbit/globe cameraControllers. Все — точечно при необходимости.
